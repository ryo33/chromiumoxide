        ...
        loop {
            ...
            match ready!(pin.ws.poll_next_unpin(cx)) {
                Some(Ok(WsMessage::Text(text))) => {
                    match text.parse::<Message<T>>() {
                        Ok(msg) => {
                            tracing::trace!("Received {:?}", msg);
                            return Poll::Ready(Some(Ok(msg)));
                        }
                        Err(err) => {
                            tracing::debug!(target: "chromiumoxide::conn::raw_ws::parse_errors", msg = text, error = %err, "Failed to parse raw WS message");
                            let err: CdpError = err.into();
